...
The newest array-statistics helpers are `threshold_run_signal_density_band_span_gap_delta_balance_sentry(array, min, max)` and `outlier_run_signal_density_band_span_gap_delta_balance_sentry(array, min, max)`. They extend balance-secure pressure with matching-mass plus longest-run pressure and return `0` for empty, single-element, or no-match inputs.
