...
The newest array-statistics helpers are `threshold_run_signal_density_band_span_gap_delta_balance_return(array, min, max)` and `outlier_run_signal_density_band_span_gap_delta_balance_return(array, min, max)`. They extend repeat pressure with matching-mass plus matching-run-count pressure and return `0` for empty, single-element, or no-match inputs.
