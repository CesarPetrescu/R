...
programs such as `1 + 2 * 3`, `let x = 2 + 3; x * 4`, and
`1 + 2; 3 * 4; 5 + 6` from a C host fixture. The evaluator supports `+`, `*`,
multiplication precedence, single-scope `let` bindings, identifier lookup,
semicolon-separated expression-statement sequencing that returns the final
expression value, and stable undefined-identifier diagnostics. The pytest suite
compiles that C runtime with `cc -std=c99 -Wall -Wextra -Werror` so the showcase
proves end-to-end interpreted expressions, bindings, and sequencing before
...
```json
{"active_blockers": [], "completed_backlog_items": 100, "has_active_blockers": false, "next_backlog_item": null, "open_backlog_items": 0, "priority_backlog_groups": {"P0": {"completed": 4, "next_item": null, "open": 0}, "P1": {"completed": 36, "next_item": null, "open": 0}, "P2": {"completed": 60, "next_item": null, "open": 0}}, "project_name": "R"}
```
