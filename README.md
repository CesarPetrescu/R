...
array literals with integer elements, checked integer indexing, and `len(...)` array length inspection for small collection-style programs,
...
loop-driven mutation, `break` exits, `continue` iteration skips, `match` integer dispatch/default arms, array literal indexing with checked out-of-bounds diagnostics, `len(...)` array length inspection with non-array diagnostics, named function calls/argument binding, block-local helper functions with lexical binding visibility and non-leakage,
...
```json
{"active_blockers": [], "completed_backlog_items": 124, "has_active_blockers": false, "next_backlog_item": null, "open_backlog_items": 0, "priority_backlog_groups": {"P0": {"completed": 4, "next_item": null, "open": 0}, "P1": {"completed": 60, "next_item": null, "open": 0}, "P2": {"completed": 60, "next_item": null, "open": 0}}, "project_name": "R"}
```
