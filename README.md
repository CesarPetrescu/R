...
The newest array-statistics helpers are `threshold_run_signal_density_band_span_gap_delta_balance_claim(array, min, max)`, `outlier_run_signal_density_band_span_gap_delta_balance_claim(array, min, max)`, `threshold_run_signal_density_band_span_gap_delta_balance_assert(array, min, max)`, and `outlier_run_signal_density_band_span_gap_delta_balance_assert(array, min, max)`. They extend the balance helper chain with matching-mass plus matching-run-count claim pressure followed by matching-mass plus transition-count assert pressure, and return `0` for empty, single-element, or no-match inputs.
...
```json
{"active_blockers": [], "completed_backlog_items": 543, "has_active_blockers": false, "next_backlog_item": null, "open_backlog_items": 0, "priority_backlog_groups": {"P0": {"completed": 4, "next_item": null, "open": 0}, "P1": {"completed": 254, "next_item": null, "open": 0}, "P2": {"completed": 285, "next_item": null, "open": 0}}, "project_name": "R"}
```
