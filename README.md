...
host fixture. Recent signal-density band helpers include `threshold_run_signal_density_band_span_gap_delta_balance_fen(array, min, max)` and `outlier_run_signal_density_band_span_gap_delta_balance_fen(array, min, max)` for matching-mass plus transition-count fen pressure, and `threshold_run_signal_density_band_span_gap_delta_balance_pool(array, min, max)` and `outlier_run_signal_density_band_span_gap_delta_balance_pool(array, min, max)` for matching-mass plus matching-run-count pool pressure. The evaluator supports `+`, `-`, `*`, `/`, `%`, unary boolean negation `!`,
...
```json
{"active_blockers": [], "completed_backlog_items": 381, "has_active_blockers": false, "next_backlog_item": null, "open_backlog_items": 0, "priority_backlog_groups": {"P0": {"completed": 4, "next_item": null, "open": 0}, "P1": {"completed": 254, "next_item": null, "open": 0}, "P2": {"completed": 123, "next_item": null, "open": 0}}, "project_name": "R"}
```
