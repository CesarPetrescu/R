...
safety before larger statement forms or runtime objects are added. Recent signal-density band helpers also include `threshold_run_signal_density_band_span_gap_delta_balance_mesh(array, min, max)` and `outlier_run_signal_density_band_span_gap_delta_balance_mesh(array, min, max)` for matching-mass plus transition-count mesh pressure on top of the balance-braid score.
