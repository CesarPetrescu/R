...
host fixture. Recent signal-density band helpers also include `threshold_run_signal_density_band_span_gap_delta_balance_marsh(array, min, max)` and `outlier_run_signal_density_band_span_gap_delta_balance_marsh(array, min, max)` for matching-mass plus transition-count marsh pressure on top of the balance-lagoon score. The evaluator supports `+`, `-`, `*`, `/`, `%`, unary boolean negation `!`,
...
safety before larger statement forms or runtime objects are added. Recent signal-density band helpers also include `threshold_run_signal_density_band_span_gap_delta_balance_marsh(array, min, max)` and `outlier_run_signal_density_band_span_gap_delta_balance_marsh(array, min, max)` for matching-mass plus transition-count marsh pressure on top of the balance-lagoon score.
...
```json
{"active_blockers": [], "completed_backlog_items": 376, "has_active_blockers": false, "next_backlog_item": null, "open_backlog_items": 0, "priority_backlog_groups": {"P0": {"completed": 4, "next_item": null, "open": 0}, "P1": {"completed": 254, "next_item": null, "open": 0}, "P2": {"completed": 118, "next_item": null, "open": 0}}, "project_name": "R"}
```
