...
- `runtime/include/rustic.h` and `runtime/rustic.c` expose `rustic_eval_expression(...)`, a C-hosted Rust-like integer expression evaluator with whitespace skipping, `+`, `*`, multiplication precedence, parenthesized expressions that override precedence, `let` bindings, identifier lookup, semicolon-separated expression-statement sequencing that returns the final expression value, assignment/mutation of existing bindings, equality comparisons (`==`) that return `1` for true and `0` for false, block expressions with nested lexical scopes and final-expression values, undefined-identifier errors for reads and assignment targets, unmatched-parenthesis errors, unclosed-block errors, malformed equality diagnostics, and stable parse-error status messages; `tests/fixtures/rustic_expression_driver.c` demonstrates the API end to end.
