...
- `python3 -m r_project --root <path> --write-readme-schema-examples` rewrites the README compact memory-overlap JSON Schema example fence in place from current generated schema output; add `--dry-run-readme-schema-examples` to print the refreshed README without modifying it.
