...
- Current implementation: tested Python scaffold with `r_project` analyzer and installable `r-project`/`r-project-lint` CLIs, including per-priority backlog summaries, README example drift tests, on-demand drift checks, generated example fences, and in-place plus dry-run README example writer modes, vector memory-layout padding helpers with explicit invalid-alignment and overflow-limit errors, C-like struct memory layout helpers with explicit invalid-alignment and overflow-limit errors, composite layout fields for nesting struct/vector layouts into larger runtime objects, named layout renderers with optional symbolic field tags/provenance metadata, opt-in recursive child layout expansion for stable memory-map debugging, half-open byte-span summaries for quick runtime overlap diagnostics, recursive flattened byte spans for fully qualified nested range comparisons, byte-span filters for narrowing diagnostics by qualified names and tags, leaf-only byte-span suppression for focusing reports on concrete child ranges, a byte-span overlap detector for intersecting runtime ranges, Markdown overlap renderers for human-readable diagnostics, grouped overlap reports by shared provenance tag or qualified-name prefix for larger trace reviews, compact grouped overlap totals for dashboards, Markdown grouped-total tables for PR comments and dashboards, threshold helpers for grouped overlap-count/intersecting-byte dashboard budgets, Markdown threshold violation tables for dashboard gates and PR comments, fixture-backed Markdown and JSON CLI demo output for threshold violations by tag or qualified-name prefix depth with optional custom overlap-count and intersecting-byte budgets and span filters by qualified-name prefix or provenance tag, fixture-backed Markdown and JSON CLI demo output for compact grouped overlap totals by tag or qualified-name prefix depth with the same span filters, JSON Schema definitions and an on-demand fixture drift check for memory-overlap demo payloads, release/versioning notes, and AGPL-3.0-or-later licensing.
