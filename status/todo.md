...
1. Add richer run-contrast summary helpers such as `threshold_run_contrast_delta(array, min, max)` and `outlier_run_contrast_delta(array, min, max)` that compare contrast scores against run deltas, with empty/single-element/no-match behavior, invalid argument diagnostics, composition with `threshold_run_contrast(...)`/`outlier_run_contrast(...)`, and array-slot cleanup.
