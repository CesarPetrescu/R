...
1. Add additional collection validation/scoring helpers such as `weighted_score(array, fn)` or `histogram_distance_score(values, counts, expected)`-style scalar validations, with empty-array behavior, invalid argument diagnostics, composition with `histogram_pairs_score(...)`, and array-slot cleanup.
