...
1. Add richer threshold trend composition helpers such as `threshold_run_score(array, min, max)` or `outlier_run_count(array, min, max)`, with empty-array behavior, invalid argument diagnostics, composition with `threshold_run_count(...)`/`outlier_streak(...)`, and array-slot cleanup.
