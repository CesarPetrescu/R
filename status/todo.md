...
1. Add additional collection validation/scoring helpers such as `histogram_distance_score(values, counts, expected)` or thresholded scoring helpers, with empty-array behavior, invalid argument diagnostics, composition with `weighted_score(...)`/`histogram_pairs_score(...)`, and array-slot cleanup.
