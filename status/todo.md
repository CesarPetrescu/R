...
1. Add another concrete Rustic product-depth helper pair such as `threshold_run_signal_density_band_ratio(array, min, max)` and `outlier_run_signal_density_band_ratio(array, min, max)` that compare density-band scores against matching run counts, with empty/single-element/no-match behavior, invalid argument diagnostics, composition fixtures, and array-slot cleanup.
