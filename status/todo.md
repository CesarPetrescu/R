...
3. Continue extending the Rustic interpreter with a bounded, test-first product-depth helper family after the balance-sample helpers; the next identifier-safe suffix is `case` (61-character threshold helper), with acceptance coverage for direct/edge behavior, invalid arguments, the composed showcase, and 65-iteration cleanup diagnostics.
