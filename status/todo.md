...
1. After the authenticated review gate is restored, merge the existing PR queue before selecting new work: #535 adds `sentry`, `warden`, and `watch`, #537 adds `keeper`, #539 adds `patrol`, and #536 migrates the automation indexes; #538 tracks the reviewer-gate evidence. Do not duplicate those open PRs; `status/missing-features.md` has no unchecked item on `main`, so choose the next concrete interpreter feature only after this queue is resolved.
