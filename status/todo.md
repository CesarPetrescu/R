...
1. Add richer collection composition showcase helpers to Rustic, such as `zip_with(array, array, fn)` for bounded pairwise integer composition without introducing pair-valued arrays, with tests proving length behavior, argument diagnostics, composition with `map_indexed`/`filter_indexed`/`fold`, and array-slot cleanup.
