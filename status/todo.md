...
1. Add richer threshold distribution summarizers such as `threshold_run_length_score(array, min, max)` or `outlier_run_length_score(array, min, max)` that consume/compose with `threshold_run_lengths(...)` and `outlier_run_lengths(...)`, with empty-array behavior, invalid argument diagnostics, and array-slot cleanup.
