...
1. Add another concrete Rustic product-depth helper pair such as `threshold_run_signal_density_band_gap(array, min, max)` and `outlier_run_signal_density_band_gap(array, min, max)` that compare density-band-ratio scores against transition-balance, with empty/single-element/no-match behavior, invalid argument diagnostics, composition fixtures, and array-slot cleanup.
