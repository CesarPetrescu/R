...
1. Add richer threshold trend validation helpers such as `threshold_run_count(array, min, max)` or `outlier_streak(array, min, max)`, with empty-array behavior, invalid argument diagnostics, composition with `threshold_window_count(...)`/`outlier_score(...)`, and array-slot cleanup.
