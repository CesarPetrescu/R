...
1. Add thresholded collection validation helpers such as `threshold_count(array, min, max)` or `histogram_within_distance(values, counts, expected, limit)`, with empty-array behavior, invalid argument diagnostics, composition with `histogram_distance_score(...)`/`weighted_score(...)`, and array-slot cleanup.
