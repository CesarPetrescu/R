...
1. Add threshold run balance helpers such as `threshold_run_delta(array, min, max)` or `outlier_run_delta(array, min, max)` that compare longest/shortest run extrema into a scalar stability score, with empty-array/no-run behavior, invalid argument diagnostics, composition with `threshold_shortest_run(...)`/`outlier_longest_run(...)`, and array-slot cleanup.
