...
- [x] Add threshold-run signal density band helpers to Rustic so showcase programs can use `threshold_run_signal_density_band(array, min, max)` and `outlier_run_signal_density_band(array, min, max)` to compare signal-density gaps against matching run-length extrema, with empty/single-element/no-match behavior, invalid argument diagnostics, composition fixtures, and array-slot cleanup.
