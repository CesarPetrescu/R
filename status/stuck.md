...
- 2026-08-14: PR #539 (`ai/r/rustic-balance-patrol`) is verified and open at head `8460447cb48ca9f4b53088e5ed75858e5ce13b9b`, but `/usr/local/bin/r-verify-ai-review 539` failed closed with exit `1` and `{"ok":false,"reason":"review_gate_disabled_pending_isolated_reviewer_identity"}`. Keep the PR open and do not merge until the authenticated isolated reviewer gate is enabled and the verifier returns exit `0` with `{"ok":true}` for the current head.
