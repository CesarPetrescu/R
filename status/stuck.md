...
- 2026-08-14: PR #539 (`ai/r/rustic-balance-patrol`) is verified and open at head `8eb3f14a3f46edb6dd64bde2567ca5eab3453af6`, but `/usr/local/bin/r-verify-ai-review 539` failed closed with exit `1` and `{"ok":false,"reason":"review_gate_disabled_pending_isolated_reviewer_identity"}`. Keep the PR open and do not merge until the authenticated isolated reviewer gate is enabled and the verifier returns exit `0` with `{"ok":true}` for the current head.
